...
D_W1 = tf.get_variable("D_W1", shape=[784, 3],
           initializer=tf.contrib.layers.xavier_initializer())
D_b1 = tf.Variable(tf.zeros(shape=[3]), name='D_b1')

D_W2 = tf.get_variable("D_W2", shape=[3, 1],
           initializer=tf.contrib.layers.xavier_initializer())
...
with sess.as_default():   # or `with sess:` to close on exit
    for x in range(0, 20000):
        X_mb, _ = mnist.train.next_batch(32)
        if x % 2000 == 0:
            ...
            plt.axis('off')
            plt.show()
        _, D_loss_curr = sess.run([D_solver, D_loss], feed_dict={X: X_mb, Z: sample_Z(32, 100)})
        _, G_loss_curr = sess.run([G_solver, G_loss], feed_dict={Z: sample_Z(32, 100)})